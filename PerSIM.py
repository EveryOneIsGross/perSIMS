...
""")
    ...
    def initialize_items(self):
        self.items = {
            ...
            'V': Item(symbol='V', name='TV', stats={"fun": 8, "comfort": 2, "environment": 1}, price=400, operating_cost=3, energy_cost=1, weight=1, 
            ...
            'W': Item(symbol='W', name='Work', stats={"social": 10, "environment": 1, "comfort": 2, "energy": 1}, 
                      price=0, energy_cost=1, weight=3, 
    ...
    def simulate_step(self):
        ...
        # First, check for critical energy levels
        if self.sim.needs["energy"] <= 2:
            bed_location = self.find_item_for_need("energy")
            if bed_location:
                self.move_towards(bed_location)
                print("The Sim is tired and is moving towards the bed.")
                self.current_activity = "moving towards Bed"
                self.current_situation = "You are heading towards your cozy bed, feeling the pull of rest."
                self.log_metrics()
                return  # End the step here to avoid conflicting actions

        # If energy isn't critical, proceed with normal activities
        if self.sim.current_item:
            ...
            if target_item:
                self.move_towards(target_item)
                self.current_activity = f"moving towards {self.house.map[target_item[1]][target_item[0]]['name']}"
                self.current_situation = "You are heading towards an item to fulfill your needs."
            else:
